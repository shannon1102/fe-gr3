...
export default function SuggestFriendList() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({
        ...
        const url =  `${process.env.REACT_APP_BASE_URL}/friend/get_list_suggested_friends?`+ params
  ...
  return (
    ...
    >
      <Grid container spacing={3}>
        {(suggestUser?.length >0) &&suggestUser.map((user)=>
          <Grid item xs={2} key={user.user_id}>
            <SuggestFriend user={user} curUser={currentUser}> </SuggestFriend>
          </Grid>
